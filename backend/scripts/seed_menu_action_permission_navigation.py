# E:\Audit\AMS\backend\scripts\seed_menu_action_permission_navigation.py
...
PERMISSIONS = [
    {
        ...
        "action": "view",
        "description": "View menu action permission mapping page.",
    },
    {
        ...
        "resource_key": "menu_action_permission",
        "action": "assign",
        "description": "Assign permission to menu action.",
    },
    {
        ...
        "resource_key": "menu_action_permission",
        "action": "remove",
        "description": "Remove permission from menu action.",
    },
]
...
MENU_ACTION_PERMISSION_MENU = {
    "menu_key": "menu_action_permission",
    "menu_title": "Menu Action Permissions",
    "route_path": "/security/menu-action-permissions",
    "icon": "KeyRound",
    "permission_key": "menu.menu_action_permission.view",
    "sort_order": 37,
}

MENU_ACTION_PERMISSION_ACTIONS = [
    {
        "action_key": "assign",
...
async def get_or_create_menu(db, group_id: int) -> Menu:
    result = await db.execute(
        select(Menu).where(
            Menu.menu_key == MENU_ACTION_PERMISSION_MENU["menu_key"]
        )
    )
    ...
    if menu:
        ...
        menu.menu_title = MENU_ACTION_PERMISSION_MENU["menu_title"]
        menu.route_path = MENU_ACTION_PERMISSION_MENU["route_path"]
        menu.icon = MENU_ACTION_PERMISSION_MENU["icon"]
        menu.permission_key = MENU_ACTION_PERMISSION_MENU["permission_key"]
        menu.sort_order = MENU_ACTION_PERMISSION_MENU["sort_order"]
        menu.menu_level = 1
        menu.is_expandable = False
        menu.is_visible = True
        menu.is_active = True
        menu.updated_by = SYSTEM_USER
        print("Menu updated: menu_action_permission")
        return menu

    menu = Menu(
        navigation_group_id=group_id,
        parent_menu_id=None,
        menu_key=MENU_ACTION_PERMISSION_MENU["menu_key"],
        menu_title=MENU_ACTION_PERMISSION_MENU["menu_title"],
        route_path=MENU_ACTION_PERMISSION_MENU["route_path"],
        icon=MENU_ACTION_PERMISSION_MENU["icon"],
        permission_key=MENU_ACTION_PERMISSION_MENU["permission_key"],
        sort_order=MENU_ACTION_PERMISSION_MENU["sort_order"],
        menu_level=1,
        is_expandable=False,
        is_visible=True,
        is_active=True,
    ...
    print("Menu created: menu_action_permission")
    return menu
...
async def get_or_create_menu_action(
    ...
    if action:
        ...
        print(f"Menu action updated: menu_action_permission.{action.action_key}")
        return action
    ...
    print(f"Menu action created: menu_action_permission.{action.action_key}")
    return action
...
async def seed_menu_action_permission_navigation() -> None:
    async with AsyncSessionLocal() as db:
        ...
        for action_data in MENU_ACTION_PERMISSION_ACTIONS:
    ...
    print("Menu Action Permission navigation seed completed.")

if __name__ == "__main__":
    asyncio.run(seed_menu_action_permission_navigation())