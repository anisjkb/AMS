...
CONTACT_TYPES = [
    ("001", "Primary Contact", "Main communication contact."),
    ("002", "Authorized Representative", "Official authorized representative."),
    ("003", "Finance Contact", "Accounts and finance contact."),
    ("004", "Audit Coordinator", "Audit coordination contact."),
    ("005", "Tax / VAT Contact", "Tax, VAT and regulatory contact."),
    ("006", "Compliance Contact", "Compliance or legal contact."),
    ("007", "Factory Contact", "Factory or plant contact."),
    ("008", "IT Contact", "IT system or data contact."),
]
...
CONTACT_MENU = {
    "menu_key": "audit_entity_contact",
    "menu_title": "Entity Contacts",
    "route_path": "/audit-entity-contacts",
    "icon": "Contact",
    "permission_key": "menu.audit_entity_contact.view",
    "sort_order": 45,
}
...
PERMISSIONS = [
    {
        "permission_key": "menu.audit_entity_contact.view",
        "resource_type": "menu",
        "resource_key": "audit_entity_contact",
        "action": "view",
        "description": "View audit entity contact page.",
    },
    {
        "permission_key": "api.audit_entity_contact.create",
        "resource_type": "api",
        "resource_key": "audit_entity_contact",
        "action": "create",
        "description": "Create audit entity contact.",
    },
    {
        "permission_key": "api.audit_entity_contact.update",
        "resource_type": "api",
        "resource_key": "audit_entity_contact",
        "action": "update",
        "description": "Update audit entity contact.",
    },
    {
        "permission_key": "api.audit_entity_contact.delete",
        "resource_type": "api",
        "resource_key": "audit_entity_contact",
        "action": "delete",
        "description": "Deactivate audit entity contact.",
    },
    {
        "permission_key": "api.audit_entity_contact.restore",
        "resource_type": "api",
        "resource_key": "audit_entity_contact",
        "action": "restore",
        "description": "Restore audit entity contact.",
    },
    {
        "permission_key": "api.audit_entity_contact.permanent_delete",
        "resource_type": "api",
        "resource_key": "audit_entity_contact",
        "action": "permanent_delete",
        "description": "Permanently delete audit entity contact.",
    },
    {
        "permission_key": "button.audit_entity_contact.create",
        "resource_type": "button",
        "resource_key": "audit_entity_contact",
        "action": "create",
        "description": "Show create contact button.",
    },
    {
        "permission_key": "button.audit_entity_contact.update",
        "resource_type": "button",
        "resource_key": "audit_entity_contact",
        "action": "update",
        "description": "Show update contact button.",
    },
    {
        "permission_key": "button.audit_entity_contact.inactive",
        "resource_type": "button",
        "resource_key": "audit_entity_contact",
        "action": "inactive",
        "description": "Show inactive contact button.",
    },
    {
        "permission_key": "button.audit_entity_contact.delete",
        "resource_type": "button",
        "resource_key": "audit_entity_contact",
        "action": "delete",
        "description": "Show delete contact button.",
    },
    {
        "permission_key": "button.audit_entity_contact.restore",
        "resource_type": "button",
        "resource_key": "audit_entity_contact",
        "action": "restore",
        "description": "Show restore contact button.",
    },
    {
        "permission_key": "button.audit_entity_contact.permanent_delete",
        "resource_type": "button",
        "resource_key": "audit_entity_contact",
        "action": "permanent_delete",
        "description": "Show permanent delete contact button.",
    },
    {
        "permission_key": "button.audit_entity_contact.export",
        "resource_type": "button",
        "resource_key": "audit_entity_contact",
        "action": "export",
        "description": "Show export contact button.",
    },
    {
        "permission_key": "button.audit_entity_contact.import",
        "resource_type": "button",
        "resource_key": "audit_entity_contact",
        "action": "import",
        "description": "Show import contact button.",
    },
]


MENU_ACTIONS = [
    ("create", "Create", "button.audit_entity_contact.create", "green", "Plus", 10),
    ("update", "Update", "button.audit_entity_contact.update", "blue", "Pencil", 20),
    ("inactive", "Inactive", "button.audit_entity_contact.inactive", "red", "Trash2", 30),
    ("delete", "Delete", "button.audit_entity_contact.delete", "red", "Trash2", 35),
    ("restore", "Restore", "button.audit_entity_contact.restore", "green", "RotateCcw", 40),
    (
        "permanent_delete",
        "Permanent Delete",
    ...
    ("export", "Export", "button.audit_entity_contact.export", "slate", "Download", 60),
    ("import", "Import", "button.audit_entity_contact.import", "slate", "Upload", 70),
]


async def seed_contact_types(db) -> None:
    for code, name, description in CONTACT_TYPES:
        result = await db.execute(
            select(AuditEntityContactType).where(
                AuditEntityContactType.contact_type_code == code
            )
        )
        contact_type = result.scalar_one_or_none()

        if contact_type:
            contact_type.contact_type_name = name
            contact_type.description = description
            contact_type.is_active = True
            contact_type.updated_by = SYSTEM_USER
            print(f"Contact type updated: {code} - {name}")
            continue

        contact_type = AuditEntityContactType(
        ...
        print(f"Contact type created: {code} - {name}")
...
async def get_or_create_menu(db, group_id: int) -> Menu:
    result = await db.execute(
        select(Menu).where(Menu.menu_key == CONTACT_MENU["menu_key"])
    )
    ...
    if menu:
        ...
        menu.menu_title = CONTACT_MENU["menu_title"]
        menu.route_path = CONTACT_MENU["route_path"]
        menu.icon = CONTACT_MENU["icon"]
        menu.permission_key = CONTACT_MENU["permission_key"]
        menu.sort_order = CONTACT_MENU["sort_order"]
        menu.menu_level = 1
        menu.is_expandable = False
        menu.is_visible = True
        menu.is_active = True
        menu.updated_by = SYSTEM_USER
        print("Menu updated: audit_entity_contact")
        return menu

    menu = Menu(
        navigation_group_id=group_id,
        parent_menu_id=None,
        menu_key=CONTACT_MENU["menu_key"],
        menu_title=CONTACT_MENU["menu_title"],
        route_path=CONTACT_MENU["route_path"],
        icon=CONTACT_MENU["icon"],
        permission_key=CONTACT_MENU["permission_key"],
        sort_order=CONTACT_MENU["sort_order"],
        menu_level=1,
        is_expandable=False,
        is_visible=True,
        is_active=True,
    ...
    print("Menu created: audit_entity_contact")
    return menu
...
async def seed_audit_entity_contact_navigation() -> None:
    async with AsyncSessionLocal() as db:
        await seed_contact_types(db)
        ...
        await get_or_create_menu_permission(
            db=db,
            menu_id=menu.id,
            permission_id=permission_by_key[CONTACT_MENU["permission_key"]].id,
        )
    ...
    print("Audit Entity Contact seed completed.")


if __name__ == "__main__":
    asyncio.run(seed_audit_entity_contact_navigation())
